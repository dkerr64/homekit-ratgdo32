...
> [!NOTE]
> As of Firmware v3.3.0 most of the source code for the ESP8266 and ESP32 versions have been merged. This allows for easier maintenance and equivalent features (where supported by the hardware).
...
> [!WARNING]
> The soft AP page has an _advanced_ mode. If you select this then the full list of discovered networks are shown, including same network SSID's broadcast by multiple access points. Selecting a WiFi network in advanced mode locks the device to a specific WiFi access point by its **unique hardware BSSID**. If that access point goes offline, or you replace it, then the device will **NOT connect** to WiFi. **Use advanced mode with extreme caution**. **Not currently implemented on ratgdo32 boards.**
